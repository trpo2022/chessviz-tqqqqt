...
CTEST(check_move, uncorrect_chess_hist)
{
    ...
    int tab[8][8];
    start_table(tab);
    int x = 4, y = 1, x1 = 4, y1 = 4, f = 1,n1=0,n2=0,n3=0, er = 0, m=0;
    move_figure(m,1,x,y,x1,y1,&f,&n1,&n2,text,1,0,&n3,tab,&er,file);
    const int res_error = 2;
    fclose(file);
    ASSERT_EQUAL(er, res_error);
}
...
CTEST(check_move, piece_not_in_pos)
{
    ...
    int tab[8][8];
    start_table(tab);
    int x = 4, y = 2, x1 = 4, y1 = 3, f = 1,n1=0,n2=0,n3=0, er = 0, m=0;
    move_figure(m,1,x,y,x1,y1,&f,&n1,&n2,text,1,0,&n3,tab,&er,file);
